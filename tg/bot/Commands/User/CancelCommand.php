...
class CancelCommand extends BaseUserCommand
{
    ...
    {
        $this->description = \Yii::t('general', 'Прерывает текущие действия с ботом');
    ...
    {
        $text = \Yii::t('general', 'Нет активных действий, которые можно завершить!');
        ...
        if ($conversation_command = $conversation->getCommand()) {
            $conversation->cancel();
            $text = \Yii::t('general', 'Действие {command} прервано!', ['command' => $conversation_command]);
        }
    ...
    {
        return $this->removeKeyboard(\Yii::t('general', 'Нечего отменять :('));
    }
}
